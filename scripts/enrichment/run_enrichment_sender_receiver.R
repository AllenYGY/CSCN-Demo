# Sender/Receiver GO & KEGG enrichment (minimal deps)
...
for (method_dir in method_dirs) {
  ...
  input_dir <- file.path(method_dir, "enrichment_inputs")
  if (!dir.exists(input_dir)) next

  files <- list.files(input_dir, pattern = "__.*\\.csv$", full.names = TRUE)
  ...
  for (file in files) {
    ...
    out_dir <- file.path(method_dir, "enrichment_results", paste0(group, "__", role))
    prefix <- paste0(method_name, "_", group, "_", role)
    message(paste0("\n=== ", prefix, " ==="))
    run_enrichment(genes, out_dir, prefix)
  }
}

summary_file <- file.path(input_root, "enrichment_results_summary.csv")
write.csv(summary_rows, file = summary_file, row.names = FALSE)
