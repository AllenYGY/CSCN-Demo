...
resolve_default_data_dir <- function() {
  candidates <- c(
    file.path(REPO_ROOT, "data", "GSE132465"),
    file.path(REPO_ROOT, "data", "GSE32465")
  )
...
parse_args <- function(args) {
  parsed <- list(
    ...
    min_count = 10L,
    min_samples = 2L,
    out_prefix = "deseq2_tumor_epithelial_vs_normal_epithelial"
  )
...
print_help <- function() {
  cat(
    paste(
      "Usage:",
      "Rscript run_deseq2_GSE132465.R [options]",
      "",
      "Options:",
      ...
      "  --out-prefix PREFIX     Output prefix. Default: deseq2_tumor_epithelial_vs_normal_epithelial",
