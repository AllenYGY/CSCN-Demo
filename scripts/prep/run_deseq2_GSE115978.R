...
default_data_dir <- normalizePath(file.path(REPO_ROOT, "data", "GSE115978"), winslash = "/", mustWork = FALSE)
...
parse_args <- function(args) {
  parsed <- list(
    ...
    min_count = 10L,
    min_samples = 2L,
    out_prefix = "deseq2_malignant_treatment_naive_vs_post"
  )
...
print_help <- function() {
  cat(
    paste(
      "Usage:",
      "Rscript run_deseq2_GSE115978.R [options]",
      "",
      "Options:",
      sprintf("  --data-dir PATH         Dataset directory. Default: %s/data/GSE115978", REPO_ROOT),
      "  --case-group NAME       Case disease label. Default: post",
      "  --control-group NAME    Control disease label. Default: naive",
      ...
      "  --out-prefix PREFIX     Output prefix. Default: deseq2_malignant_treatment_naive_vs_post",
...
ensure_required_columns(metadata, c("sample", "disease", "cohort"), "metadata")
