...
if (nzchar(repo_root_deseq)) {
  source(file.path(repo_root_deseq, "scripts", "prep", "scp259_utils.R"), chdir = FALSE)
} else {
  script_path_deseq <- normalizePath(sub("^--file=", "", file_arg_deseq[[1L]]), winslash = "/", mustWork = TRUE)
  source(file.path(dirname(script_path_deseq), "scp259_utils.R"), chdir = FALSE)
}

suppressPackageStartupMessages({
  if (!requireNamespace("DESeq2", quietly = TRUE)) {
    stop("Package 'DESeq2' is required. Install it with BiocManager::install('DESeq2').", call. = FALSE)
  }
  library(DESeq2)
})
...
parse_args <- function(args) {
  parsed <- list(
    ...
    min_count = 10L,
    min_samples = 2L,
    out_prefix = sprintf("deseq2_%s", RUN_SLUG_SCP259)
  )
...
print_help <- function() {
  cat(
    paste(
      "Usage:",
      "Rscript run_deseq2_SCP259.R [options]",
      "",
      "Options:",
      sprintf("  --data-dir PATH         Dataset directory. Default: %s", default_data_dir_scp259()),
      "  --case-group NAME       Case group label. Default: Inflamed",
      "  --control-group NAME    Control group label. Default: Healthy",
      ...
      "  --out-prefix PREFIX     Output prefix. Default: deseq2_inflamed_vs_healthy_crypt_prolif_epi",
...
count_path <- file.path(output_dir, sprintf("count_matrix_%s_by_subject.csv", RUN_SLUG_SCP259))
metadata_path <- file.path(output_dir, sprintf("metadata_%s_by_subject.csv", RUN_SLUG_SCP259))
...
validate_required_file_scp259(count_path, "count matrix")
validate_required_file_scp259(metadata_path, "metadata")
...
ensure_required_columns(metadata, c("Subject", "Health", "n_cells"), "metadata")

metadata <- metadata[metadata$Health %in% c(args$control_group, args$case_group), , drop = FALSE]
if (nrow(metadata) == 0L) {
  stop("No metadata rows remain after filtering to requested health groups", call. = FALSE)
}

sample_ids <- unique(metadata$Subject)
missing_samples <- setdiff(sample_ids, colnames(count_df))
if (length(missing_samples) > 0L) {
  stop(
    sprintf("Count matrix is missing metadata subjects: %s", paste(missing_samples, collapse = ", ")),
    call. = FALSE
  )
}
...
metadata <- metadata[match(sample_ids, metadata$Subject), , drop = FALSE]
rownames(metadata) <- metadata$Subject
metadata$Health <- factor(metadata$Health, levels = c(args$control_group, args$case_group))
...
cat("Subjects:", ncol(dds), "\n\n")

dds <- DESeq(dds, sfType = "poscounts")
res <- results(dds, contrast = c("Health", args$case_group, args$control_group))
