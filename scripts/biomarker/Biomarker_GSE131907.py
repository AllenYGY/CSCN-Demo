...
DATA_SET = "GSE131907"
RUN_NAME = f"{DATA_SET}_nlung_epithelial_vs_tlung_malignant"
GROUP_TO_LABEL = {"normal": 0, "cancer": 1}
GROUP_FILTERS = {
    "normal": {
...
def default_gene_list_path(output_dir: Path):
    return output_dir / "deseq2_nlung_epithelial_vs_tlung_malignant_top150_genes.csv"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run CSCN biomarker analysis for GSE131907 using nLung epithelial cells "
            "versus tLung malignant epithelial cells."
        )
    )
    parser.add_argument(
        "--data-dir",
...
def log_group_summary(summary_by_group):
    for group, summary in summary_by_group.items():
        ...
        if subtype_counts:
            ...
            preview = ", ".join(f"{name}={count}" for name, count in sorted_subtypes[:8])
...
def main():
    ...
    eligible_cells, summary_by_group = load_gse131907_grouped_cells(
